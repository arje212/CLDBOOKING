...
from .models import (
    ...
    Todo, ChatMessage, FutureProject, SiteSettings,
)
...
# ════════════════════════════════════════════════════════════════
# 🆕 ROOM COST SUMMARY EXPORT
# ════════════════════════════════════════════════════════════════

@login_required
@user_passes_test(lambda u: u.is_superuser)
def export_room_cost_excel(request):
    bookings = Booking.objects.select_related("room", "created_by").order_by("room__name", "-start")
    rooms    = Room.objects.all().order_by("name")

    wb = Workbook()

    # Sheet 1: Summary per Room
    ws1 = wb.active
    ws1.title = "Room Cost Summary"
    ws1.append(["Room", "Price/hr (PHP)", "Times Used", "Total Hours", "Total Spent (PHP)"])
    grand_total = 0
    for room in rooms:
        rb          = bookings.filter(room=room)
        times_used  = rb.count()
        total_hours = round(sum(float(b.hours_used) for b in rb), 2)
        total_spent = round(sum(float(b.total_cost) for b in rb), 2)
        grand_total += total_spent
        ws1.append([room.name, float(room.price_per_hour), times_used, total_hours, total_spent])
    ws1.append(["", "", "", "GRAND TOTAL", round(grand_total, 2)])

    # Sheet 2: Detailed per-booking usage
    ws2 = wb.create_sheet("Detailed Usage")
    ws2.append(["Room", "Booking Title", "Used By", "Date", "Start", "End", "Hours", "Cost (PHP)", "Status"])
    for b in bookings:
        ws2.append([
            b.room.name,
            b.title,
    ...
    response = HttpResponse(content_type="application/ms-excel")
    response['Content-Disposition'] = 'attachment; filename="room_cost_summary.xlsx"'
    wb.save(response)
    return response
...
def admin_dashboard(request):
    ...
    # 🆕 Per-room cost summary
    room_cost_summary = []
    for room in rooms:
        room_bookings = bookings.filter(room=room)
        times_used    = room_bookings.count()
        total_hours   = sum(float(b.hours_used) for b in room_bookings)
        total_spent   = sum(float(b.total_cost) for b in room_bookings)
        room_cost_summary.append({
            'room':        room,
            'times_used':  times_used,
            'total_hours': round(total_hours, 2),
            'total_spent': round(total_spent, 2),
        })
    grand_total_spent = round(sum(r['total_spent'] for r in room_cost_summary), 2)
    ...
    summary_cards = [
        ...
        {"label": "Total Revenue",  "count": f"P{total_revenue:,.2f}",                   "icon": "bi-cash-stack"},
    ]
    ...
    return render(request, "booking/admin_dashboard.html", {
        ...
        "total_revenue":      total_revenue,
        "site":               site,
        "room_cost_summary":  room_cost_summary,
